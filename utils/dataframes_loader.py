            ...
            try:
                with h5py.File(file_path, 'r') as f:
                    ...
                    for key in sorted(keys):
                        print(f"  - {key}")
                        keys_to_print_shape = ['HLT_data', 'HLT_latent_reps', 'HLT_AD_scores', 'HLT_model_outputs', 'region_labels','pileups']
                        if key in keys_to_print_shape:
...
# For 2024 data only
def load_dataframes_from_h5_raw_pt_phi(data_dir):
    """
    Loads from HDF5 files in a directory and put it into a dataframe.
    ...
    for filename in os.listdir(data_dir):
        if filename.endswith(".h5"):
            ...
            with h5py.File(file_path, 'r') as f:
                # Create main DataFrame with HLT_data
                df = pd.DataFrame(f['raw_HLT_data'][:], columns=columns)
